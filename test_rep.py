...
f = [
    ...
    fml(f'$L_{{ф}} = (L_{{11}} + L_{{22}} + L_{{33}}) \\div 3; \\quad '
        f'L_{{ф}} =({round(p2.l_dg0*10**3, 2)}) \\div 3 = {average(p2.l_dg0*10**3):.2f}$ мГн'),
    sp_40,
    fml(f'$L_{{фф}} = (L_{{12}} + L_{{13}} + L_{{23}}) \\div 3; \\quad '
        f'({round(p2.l_tr0*10**3, 2)}) \\div 3 = {average(p2.l_tr0)*10**3:.2f}$ мГн'),
    # fml(f'L_{{ф}} = (L_{{11}} + L_{{22}} + L_{{33}}) \\div 3; \\quad '
    #     f'({diag(p2.l[0])*10**9}) \\div 3 = {round(diag(p2.l[0]))*10**9}$ нФ'),
